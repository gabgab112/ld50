...
public class ParallaxControllerWithoutDelegate : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        if (Camera.main.transform.position.x != oldCameraPosition.x || (Camera.main.transform.position.y) != oldCameraPosition.y)
        {
            if (parallaxLayers.Count > 0)
            {
                ...
                this.MoveChildrenLayer(cameraPositionChange.x, cameraPositionChange.y);
            }
    ...
    private void MoveChildrenLayer(float positionChangeX, float positionChangeY)
    {
